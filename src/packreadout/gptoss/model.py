"""Loading a causal LM and reading its LM head (gpt-oss implementation; see the package docstring).
...
def load_model(
    ...
    """Load a causal LM in eval mode: bf16 on CUDA, fp32 elsewhere. gpt-oss: eager attention,
    the only path that implements its attention sinks, and its MXFP4 weights dequantized."""
    ...
    from transformers import Mxfp4Config  # gpt-oss: the checkpoint stores the experts in 4-bit MXFP4
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        device_map="auto" if sharded else None,
    )  # gpt-oss: eager attention (sinks), dequantized weights
    if not sharded:
        model.to(device)
    model.eval()
    return model, tok
